    ...
    <script type="text/javascript">
        ...
        window.setInterval(firstSlide, 5000, {!! $essays[0]->homes !!});
        window.setInterval(secondSlide, 4000, {!! $essays[1]->homes !!});
        window.setInterval(tirdSlide, 3000, {!! $essays[2]->homes !!});

        function firstSlide(array) 
        {
            var index = Math.floor((Math.random() * (array.length - 1)) + 1);
            var image = array[index].name;
            // jQuery('.firstSlide').fadeOut('slow');
            jQuery('.firstSlide').css({'background-image': 'url(/storage/home/' + image + ')'});
        }

        function secondSlide(array) 
        {
            var index = Math.floor((Math.random() * (array.length - 1)) + 1);
            var image = array[index].name;
            jQuery('.secondSlide').css({'background-image': 'url(/storage/home/' + image + ')'});
        }

        function tirdSlide(array) 
        {
            var index = Math.floor((Math.random() * (array.length - 1)) + 1);
            var image = array[index].name;
            jQuery('.tirdSlide').css({'background-image': 'url(/storage/home/' + image + ')'});
        }
